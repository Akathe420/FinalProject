...
class AccessToken implements JsonSerializable
{
    ...
    public static function verify(AccessToken $accessToken): bool
    {
        ...
        if ($token) {
            // We found the token in the DB
            // Check whether the token is still valid
            $birth = strtotime($token->birth); // time when token was created in DB
            $age = abs(strtotime('now') - $birth); // Get the age of the token in seconds
            if ($age < 3600) {
                return true; // Token is less than 1 hour old, so it's valid.
            }
        }
        return false;
    }
