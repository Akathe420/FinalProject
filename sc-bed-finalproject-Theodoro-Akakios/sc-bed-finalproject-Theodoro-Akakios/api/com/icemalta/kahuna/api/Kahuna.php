...
$endPoints["user"] = function (string $requestMethod, array $requestData): void {
    if ($requestMethod === 'POST') {
        ...
        $user = new User($email, $password, $firstName, $lastName, $accessLevel);
        
        // Save the user to the database
        $user = User::save($user);
        
        // Send a response back with the newly created user data
...
$endPoints["login"] = function (string $requestMethod, array $requestData): void {
    if ($requestMethod === 'POST') {
        ...
        // print_r("Hello there2");
        if ($authenticatedUser) {
            // print_r("Hello there3");
            // Generate an access token for the authenticated user
            $token = new AccessToken($authenticatedUser->getId());
            $token = AccessToken::save($token);
            // print_r("Hello there4");   
            // Send the authenticated user data and token in the response
            sendResponse([
                'user' => [
